...
def is_auto_generated(name_or_symbol):
    """
    Return True if the symbol (or name) is auto-generated by Ghidra's analysis.
    Accepts either a symbol object or a string name.
    """
    # If it's a symbol object with getSource(), check if it's from analysis
    try:
        if hasattr(name_or_symbol, "getSource"):
            return name_or_symbol.getSource() != SourceType.USER_DEFINED
        if hasattr(name_or_symbol, "getSymbol") and name_or_symbol.getSymbol():
            return name_or_symbol.getSymbol().getSource() != SourceType.USER_DEFINED
    except Exception:
        pass
    # Fallback: heuristics for string names
    name = str(name_or_symbol)
    return (
        re.match(r".*_[0-9a-fA-F]+$", name)
        or re.match(r"(?:[iuapbcdst]|ppp)Var\d+$", name)
        or re.match(r"[a-zA-Z]Ram[0-9a-fA-F]+$", name)
        or re.match(r"in_r\d+$", name)
        or re.match(r"extraout_r\d+$", name)
        or re.match(r"p[D-Z_]Var\d+$", name)
        or re.match(r"[a-zA-Z]Var\d+$", name)  # catch bVar2, cVar3, etc.
        or re.match(r"[a-zA-Z]Stack\d+.*", name)  # catch bVar2, cVar3, etc.
